...
function loadData(pcData) {
  // Combine PCA output and info by joining with portalId
  let promise = d3.tsv(infoUrl, function(d) {
    return {
      portalId : d.portal_id,
      PC1 : +pcData.find(element => element.portalId === d.portal_id).PC1,
      PC2 : +pcData.find(element => element.portalId === d.portal_id).PC2,
...
function showData(data, PC1Explained, PC2Explained) {
  // Stop spinner
  spinner.stop()
  ...
  gDot.selectAll(".point")
    ...
    .append("path")
      ...
      // Click event: open CAZy page of the select portal
...
function get_color_shape(data, group, scale) {
  // Assign color and shape based on the selected group
  // Groups are taxPhylum, taxClass, taxOrder, and ecoTerm
